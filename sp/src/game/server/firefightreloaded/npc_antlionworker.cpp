...
// base antlion stuff
ConVar	sk_antlion_worker_swipe_damage("sk_antlion_worker_swipe_damage", "0");
ConVar	sk_antlion_worker_jump_damage("sk_antlion_worker_jump_damage", "0");
ConVar  sk_antlion_worker_air_attack_dmg("sk_antlion_worker_air_attack_dmg", "0");
...
ConVar  g_test_new_antlion_worker_jump("g_test_new_antlion_worker_jump", "1", FCVAR_ARCHIVE);
ConVar	antlion_worker_easycrush("antlion_worker_easycrush", "1");
ConVar g_antlion_worker_cascade_push("g_antlion_worker_cascade_push", "1", FCVAR_ARCHIVE);
 
//ConVar g_debug_antlion_worker( "g_debug_antlion_worker", "0" );

extern ConVar bugbait_radius;

int AE_WNTLION_WALK_FOOTSTEP;
int AE_WNTLION_MELEE_HIT1;
int AE_WNTLION_MELEE_HIT2;
int AE_WNTLION_MELEE_POUNCE;
int AE_WNTLION_FOOTSTEP_SOFT;
int AE_WNTLION_FOOTSTEP_HEAVY;
int AE_WNTLION_START_JUMP;
int AE_WNTLION_BURROW_IN;
int AE_WNTLION_BURROW_OUT;
int AE_WNTLION_VANISH;
int AE_WNTLION_OPEN_WINGS;
int AE_WNTLION_CLOSE_WINGS;
int AE_WNTLION_MELEE1_SOUND;
int AE_WNTLION_MELEE2_SOUND;
int AE_WNTLION_WORKER_EXPLODE_SCREAM;
int AE_WNTLION_WORKER_EXPLODE_WARN;
int AE_WNTLION_WORKER_EXPLODE;
int AE_WNTLION_WORKER_SPIT;
int AE_WNTLION_WORKER_DONT_EXPLODE;
...
enum
{	
	SQUAD_SLOT_ANTLIONWORKER_JUMP = LAST_SHARED_SQUADSLOT,
	SQUAD_SLOT_ANTLIONWORKER_FIRE,
};
...
int ACT_WNTLION_JUMP_START;
int	ACT_WNTLION_DISTRACT;
int ACT_WNTLION_DISTRACT_ARRIVED;
int ACT_WNTLION_BURROW_IN;
int ACT_WNTLION_BURROW_OUT;
int ACT_WNTLION_BURROW_IDLE;
int	ACT_WNTLION_RUN_AGITATED;
int ACT_WNTLION_FLIP;
int ACT_WNTLION_ZAP_FLIP;
int ACT_WNTLION_POUNCE;
int ACT_WNTLION_POUNCE_MOVING;
int ACT_WNTLION_DROWN;
int ACT_WNTLION_LAND;
int ACT_WNTLION_WORKER_EXPLODE;
...
CNPC_AntlionWorker::CNPC_AntlionWorker( void )
{
	m_flIdleDelay	= 0.0f;
	m_flBurrowTime	= 0.0f;
	m_flJumpTime	= 0.0f;
...
LINK_ENTITY_TO_CLASS( npc_antlionworker, CNPC_AntlionWorker );

//==================================================
// CNPC_AntlionWorker::m_DataDesc
//==================================================

BEGIN_DATADESC( CNPC_AntlionWorker )
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_AntlionWorker::Spawn( void )
{
	Precache();
	...
	ConVar *sk_antlion_worker_health = cvar->FindVar("sk_antlion_worker_health");
	m_iHealth = sk_antlion_worker_health->GetFloat();
	...
	if ( HasSpawnFlags( SF_ANTLIONWORKER_USE_GROUNDCHECKS ) == false )
	...
	if ( m_bStartBurrowed )
	{
		...
		SetActivity( (Activity) ACT_WNTLION_BURROW_IDLE );
		SetSchedule( SCHED_ANTLIONWORKER_WAIT_FOR_UNBORROW_TRIGGER );

		SetUse( &CNPC_AntlionWorker::BurrowUse );
	}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_AntlionWorker::Activate( void )
{
...
//-----------------------------------------------------------------------------
bool CNPC_AntlionWorker::CreateVPhysics()
{
...
const char *pszAntlionWorkerGibs_Unique[NUM_ANTLION_GIBS_UNIQUE] = {
...
const char *pszAntlionWorkerGibs_Medium[NUM_ANTLION_GIBS_MEDIUM] = {
...
const char *pszAntlionWorkerGibs_Small[NUM_ANTLION_GIBS_SMALL] = {
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_AntlionWorker::Precache( void )
{
	...
	{
		PrecacheModel( pszAntlionWorkerGibs_Unique[ i ] );
	}
	for ( int i = 0; i < NUM_ANTLION_GIBS_MEDIUM; ++i )
	{
		PrecacheModel(pszAntlionWorkerGibs_Medium[i]);
	}
	for ( int i = 0; i < NUM_ANTLION_GIBS_SMALL; ++i )
	{
		PrecacheModel(pszAntlionWorkerGibs_Small[i]);
	}
...
//-----------------------------------------------------------------------------
inline CBaseEntity *CNPC_AntlionWorker::EntityToWatch( void )
{
...
//-----------------------------------------------------------------------------
void	CNPC_AntlionWorker::PopulatePoseParameters( void )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_AntlionWorker::UpdateHead( void )
{
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CNPC_AntlionWorker::FInViewCone( CBaseEntity *pEntity )
{
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CNPC_AntlionWorker::FInViewCone( const Vector &vecSpot )
{
	m_flFieldOfView = ( GetEnemy() != NULL ) ? ANTLION_VIEW_FIELD_NARROW : VIEW_FIELD_WIDE;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_AntlionWorker::CanBecomeRagdoll()
{
...
// Input  : *pVictim - 
//-----------------------------------------------------------------------------
void CNPC_AntlionWorker::Event_Killed( const CTakeDamageInfo &info )
{
	//Turn off wings
	SetWings( false );
	VacateStrategySlot();

	if ( IsCurSchedule(SCHED_ANTLIONWORKER_BURROW_IN) || IsCurSchedule(SCHED_ANTLIONWORKER_BURROW_OUT) )
	{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_AntlionWorker::MeleeAttack( float distance, float damage, QAngle &viewPunch, Vector &shove )
{
	Vector vecForceDir;
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CNPC_AntlionWorker::FindChasePosition( const Vector &targetPos, Vector &result )
{
	if ( HasSpawnFlags( SF_ANTLIONWORKER_USE_GROUNDCHECKS ) == true )
	{
		 result = targetPos;
		 return true;
	}
	...
	if ( g_debug_antlion_worker->GetInt() == 1 )
	{
...
// Input  : &testPos - 
//-----------------------------------------------------------------------------
bool CNPC_AntlionWorker::GetGroundPosition( const Vector &testPos, Vector &result )
{
	ConVar *g_debug_antlion_worker = cvar->FindVar("g_debug_antlion_worker");
	...
	if ( tr.allsolid )
	{
		if (g_debug_antlion_worker->GetInt() == 3)
		{
...
void CNPC_AntlionWorker::ManageFleeCapabilities( bool bEnable )
{
	if ( bEnable == false )
	{
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CNPC_AntlionWorker::GetPathToSoundFleePoint( int soundType )
{
	...
	if ( pHint != NULL )
	{
		...
		AI_NavGoal_t goal( vecFleeGoal, (Activity) ACT_WNTLION_RUN_AGITATED, 128, AIN_DEF_FLAGS );
		...
		if ( GetNavigator()->SetGoal( goal ) )
		{
			...
			GetNavigator()->SetArrivalActivity( (Activity) ACT_WNTLION_DISTRACT_ARRIVED );
	...
	// Find a route to that position
	AI_NavGoal_t goal( vecFleeGoal + Vector( 0, 0, 8 ), (Activity) ACT_WNTLION_RUN_AGITATED, 512, AIN_DEF_FLAGS );
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CNPC_AntlionWorker::SeenEnemyWithinTime( float flTime )
{
...
//-----------------------------------------------------------------------------
bool CNPC_AntlionWorker::InnateWeaponLOSCondition( const Vector &ownerPos, const Vector &targetPos, bool bSetConditions )
{
...
Vector WorkerVecCheckThrowTolerance( CBaseEntity *pEdict, const Vector &vecSpot1, Vector vecSpot2, float flSpeed, float flTolerance )
{
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CNPC_AntlionWorker::GetSpitVector( const Vector &vecStartPos, const Vector &vecTarget, Vector *vecOut )
{
	ConVar *sk_antlion_worker_spit_speed = cvar->FindVar("sk_antlion_worker_spit_speed");

	// antlion workers exist only in episodic.
	// Try the most direct route
	Vector vecToss = WorkerVecCheckThrowTolerance( this, vecStartPos, vecTarget, sk_antlion_worker_spit_speed->GetFloat(), (10.0f*12.0f) );

	// If this failed then try a little faster (flattens the arc)
	if ( vecToss == vec3_origin )
	{
		vecToss = WorkerVecCheckThrowTolerance(this, vecStartPos, vecTarget, sk_antlion_worker_spit_speed->GetFloat() * 1.5f, (10.0f*12.0f));
		if ( vecToss == vec3_origin )
			return false;
	}
...
// Input  : flDuration - 
//-----------------------------------------------------------------------------
void CNPC_AntlionWorker::DelaySquadAttack( float flDuration )
{
	if ( GetSquad() )
	{
...
// Input  : *pEvent - 
//-----------------------------------------------------------------------------
void CNPC_AntlionWorker::HandleAnimEvent( animevent_t *pEvent )
{
	// Handle the spit event
	if ( pEvent->event == AE_WNTLION_WORKER_SPIT )
	{
		if ( GetEnemy() )
		{
			Vector vSpitPos;
	...
	if ( pEvent->event == AE_WNTLION_WORKER_DONT_EXPLODE )
	{
		m_bDontExplode = true;
		return;
	}

	if ( pEvent->event == AE_WNTLION_WALK_FOOTSTEP )
	{
	...
	if ( pEvent->event == AE_WNTLION_MELEE_HIT1 )
	{
		QAngle qa( 20.0f, 0.0f, -12.0f );
		Vector vec( -250.0f, 1.0f, 1.0f );
		MeleeAttack( ANTLION_MELEE1_RANGE, sk_antlion_worker_swipe_damage.GetFloat(), qa, vec );
		return;
	}

	if ( pEvent->event == AE_WNTLION_MELEE_HIT2 )
	{
	...
	if ( pEvent->event == AE_WNTLION_MELEE_POUNCE )
	{
		QAngle qa( 4.0f, 0.0f, 0.0f );
		Vector vec( -250.0f, 1.0f, 1.0f );
		MeleeAttack(ANTLION_MELEE2_RANGE, sk_antlion_worker_swipe_damage.GetFloat(), qa, vec);
		return;
	}
		
	if ( pEvent->event == AE_WNTLION_OPEN_WINGS )
	{
		SetWings( true );
		return;
	}

	if ( pEvent->event == AE_WNTLION_CLOSE_WINGS )
	{
		SetWings( false );
		return;
	}

	if ( pEvent->event == AE_WNTLION_VANISH )
	{
		AddSolidFlags( FSOLID_NOT_SOLID );
		m_takedamage	= DAMAGE_NO;
		AddEffects( EF_NODRAW );
	...
	if ( pEvent->event == AE_WNTLION_BURROW_IN )
	{
		//Burrowing sound
	...
	if ( pEvent->event == AE_WNTLION_BURROW_OUT )
	{
	...
	if ( pEvent->event == AE_WNTLION_FOOTSTEP_SOFT )
	{
		EmitSound( "NPC_Antlion.FootstepSoft", pEvent->eventtime );
		return;
	}

	if ( pEvent->event == AE_WNTLION_FOOTSTEP_HEAVY )
	{
	...
	if ( pEvent->event == AE_WNTLION_MELEE1_SOUND )
	{
		EmitSound( "NPC_Antlion.MeleeAttackSingle" );
		return;
	}
	
	if ( pEvent->event == AE_WNTLION_MELEE2_SOUND )
	{
		EmitSound( "NPC_Antlion.MeleeAttackDouble" );
		return;
	}

	if ( pEvent->event == AE_WNTLION_START_JUMP )
	{
		StartJump();
		return;
	}
	...
	if ( pEvent->event == AE_WNTLION_WORKER_EXPLODE_SCREAM )
	{
		if ( GetWaterLevel() < 2 )
		{
	...
	ConVar *sk_antlion_worker_burst_radius = cvar->FindVar("sk_antlion_worker_burst_radius");
	ConVar *sk_antlion_worker_burst_damage = cvar->FindVar("sk_antlion_worker_burst_damage");

	if ( pEvent->event == AE_WNTLION_WORKER_EXPLODE_WARN )
	{
		CSoundEnt::InsertSound( SOUND_PHYSICS_DANGER, GetAbsOrigin(), sk_antlion_worker_burst_radius->GetFloat(), 0.5f, this );
		return;
	}

	if ( pEvent->event == AE_WNTLION_WORKER_EXPLODE )
	{
...
bool CNPC_AntlionWorker::IsUnusableNode(int iNodeID, CAI_Hint *pHint)
{
	bool iBaseReturn = BaseClass::IsUnusableNode( iNodeID, pHint );

	if (g_test_new_antlion_worker_jump.GetBool() == 0)
		 return iBaseReturn;
...
void CNPC_AntlionWorker::LockJumpNode( void )
{
	if (HasSpawnFlags(SF_ANTLIONWORKER_USE_GROUNDCHECKS) == false)
		 return;
	...
	if (g_test_new_antlion_worker_jump.GetBool() == false)
		 return;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_AntlionWorker::OnObstructionPreSteer( AILocalMoveGoal_t *pMoveGoal, float distClear, AIMoveResult_t *pResult )
{
	...
	{
		// HACKHACK
		CNPC_AntlionWorker *pAntlion = dynamic_cast< CNPC_AntlionWorker * > ( pBlocker );
...
bool NPC_AntlionWorker_IsAntlionWorker( CBaseEntity *pEntity )
{
	CNPC_AntlionWorker *pAntlion = dynamic_cast<CNPC_AntlionWorker *>(pEntity);

	return pAntlion ? true : false;
}

class CTraceFilterAntlionWorker : public CTraceFilterEntitiesOnly
{
public:
	CTraceFilterAntlionWorker(const CBaseEntity *pEntity) { m_pIgnore = pEntity; }
	...
	virtual bool ShouldHitEntity( IHandleEntity *pHandleEntity, int contentsMask )
	{
		...
		if (NPC_AntlionWorker_IsAntlionWorker(pEntity))
			 return true;
...
// Purpose:  
//-----------------------------------------------------------------------------
void CNPC_AntlionWorker::StartTask( const Task_t *pTask )
{
	switch ( pTask->iTask ) 
	{
	case TASK_ANTLIONWORKER_FIND_COVER_FROM_SAVEPOSITION:
		{
			Vector coverPos;
	...
	case TASK_ANTLIONWORKER_FACE_JUMP:
		break;

	case TASK_ANTLIONWORKER_DROWN:
	{
	...
	case TASK_ANTLIONWORKER_REACH_FIGHT_GOAL:

		m_OnReachFightGoal.FireOutput( this, this );
		TaskComplete();
		break;

	case TASK_ANTLIONWORKER_DISMOUNT_NPC:
		{
			...
			{
				trace_t trace;
				CTraceFilterAntlionWorker traceFilter( this );
				...
				ResetActivity();
				SetActivity( (Activity) ACT_WNTLION_FLIP );
			}
			else
			{
				// Dead or gone now
	...
	case TASK_ANTLIONWORKER_FACE_BUGBAIT:
	...
	case TASK_ANTLIONWORKER_GET_PATH_TO_BUGBAIT:
		{
			//Must have a saved sound
			//FIXME: This isn't assured to be still pointing to the right place, need to protect this
			if ( !m_bHasHeardSound )
			...
			{
				AI_NavGoal_t goal( goalPos, (Activity) ACT_WNTLION_RUN_AGITATED, ANTLION_BUGBAIT_NAV_TOLERANCE );
				...
				if ( GetNavigator()->SetGoal( goal, AIN_DISCARD_IF_FAIL ) == false )
				{
					...
					AI_NavGoal_t nearGoal( GOALTYPE_LOCATION_NEAREST_NODE, goalPos, (Activity) ACT_WNTLION_RUN_AGITATED, ANTLION_BUGBAIT_NAV_TOLERANCE );
	...
	case TASK_ANTLIONWORKER_WAIT_FOR_TRIGGER:
		m_flIdleDelay = gpGlobals->curtime + 1.0f;

		break;

	case TASK_ANTLIONWORKER_JUMP:
	...
	case TASK_ANTLIONWORKER_CHECK_FOR_UNBORROW:
	...
	case TASK_ANTLIONWORKER_BURROW_WAIT:
	...
	case TASK_ANTLIONWORKER_FIND_BURROW_IN_POINT:
		
		if ( FindBurrow( GetAbsOrigin(), pTask->flTaskData, ANTLION_BURROW_IN ) == false )
		{
			TaskFail( "TASK_ANTLIONWORKER_FIND_BURROW_IN_POINT: Unable to find burrow in position\n" );
		}
		else
		{
			TaskComplete();
	...
	case TASK_ANTLIONWORKER_FIND_BURROW_OUT_POINT:
		
		if ( FindBurrow( GetAbsOrigin(), pTask->flTaskData, ANTLION_BURROW_OUT ) == false )
		{
			TaskFail( "TASK_ANTLIONWORKER_FIND_BURROW_OUT_POINT: Unable to find burrow out position\n" );
		}
		else
		{
			TaskComplete();
	...
	case TASK_ANTLIONWORKER_BURROW:
		Burrow();
		TaskComplete();

		break;

	case TASK_ANTLIONWORKER_UNBURROW:
		Unburrow();
		TaskComplete();

		break;

	case TASK_ANTLIONWORKER_VANISH:
		AddEffects( EF_NODRAW );
		AddFlag( FL_NOTARGET );
		m_spawnflags |= SF_NPC_GAG;
		...
		if ( pTask->flTaskData )
		{
			...
			SetThink( &CNPC_AntlionWorker::SUB_Remove );
	...
	case TASK_ANTLIONWORKER_GET_THUMPER_ESCAPE_PATH:
		{
	...
	case TASK_ANTLIONWORKER_GET_PHYSICS_DANGER_ESCAPE_PATH:
		{
...
// Input  : *pTask - 
//-----------------------------------------------------------------------------
void CNPC_AntlionWorker::RunTask( const Task_t *pTask )
{
...
bool CNPC_AntlionWorker::AllowedToBePushed( void )
{
	if ( IsCurSchedule( SCHED_ANTLIONWORKER_BURROW_WAIT ) || 
		IsCurSchedule(SCHED_ANTLIONWORKER_BURROW_IN) || 
		IsCurSchedule(SCHED_ANTLIONWORKER_BURROW_OUT) ||
		IsCurSchedule(SCHED_ANTLIONWORKER_BURROW_AWAY ) ||
		IsCurSchedule( SCHED_ANTLIONWORKER_RUN_TO_FIGHT_GOAL ) )
		return false;

	if ( IsRunningDynamicInteraction() )
		return false;

	if ( IsMoving() == false && IsCurSchedule( SCHED_ANTLIONWORKER_FLIP ) == false
...
// Output :
//-----------------------------------------------------------------------------
bool CNPC_AntlionWorker::IsJumpLegal( const Vector &startPos, const Vector &apex, const Vector &endPos ) const
{
	...
	if (HasSpawnFlags(SF_ANTLIONWORKER_USE_GROUNDCHECKS) && g_test_new_antlion_worker_jump.GetBool() == true)
	{
		trace_t	tr;
...
bool CNPC_AntlionWorker::IsFirmlyOnGround( void )
{
	if( !( GetFlags()&FL_ONGROUND ) )
		return false;
...
// Purpose: 
//-----------------------------------------------------------------------------
int CNPC_AntlionWorker::SelectFailSchedule( int failedSchedule, int failedTask, AI_TaskFailureCode_t taskFailCode )
{
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CNPC_AntlionWorker::ShouldJump( void )
{
	if ( GetEnemy() == NULL )
		return false;
	...
	{
		if (g_debug_antlion_worker->GetInt() == 2)
		{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
int CNPC_AntlionWorker::TranslateSchedule( int scheduleType )
{
	if ( ( m_hFollowTarget != NULL ) || IsAllied() )
	{
		if ( ( scheduleType == SCHED_IDLE_STAND ) || ( scheduleType == SCHED_ALERT_STAND ) )
			return SCHED_ANTLIONWORKER_BUGBAIT_IDLE_STAND;
	}
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
Activity CNPC_AntlionWorker::NPC_TranslateActivity( Activity baseAct )
{
	// Workers explode as long as they didn't drown.
	if ( ( baseAct == ACT_DIESIMPLE ) && !m_bDontExplode )
	{
		return ( Activity )ACT_WNTLION_WORKER_EXPLODE;
	}
...
int CNPC_AntlionWorker::ChooseMoveSchedule( void )
{
			...
			if ( m_hFightGoalTarget )
			{
				...
				if ( targetDist > 256 )
				{
					...
					if ( FindChasePosition( targetPos, testPos ) )
					{
						...
						return SCHED_ANTLIONWORKER_RUN_TO_FIGHT_GOAL;
					}
				}
			}
		}
	...
	// Following a goal
	case ANTLIONWORKER_MOVE_FOLLOW:
		{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_AntlionWorker::ZapThink( void )
{
	CEffectData	data;
	...
	{
		SetContextThink( &CNPC_AntlionWorker::ZapThink, gpGlobals->curtime + random->RandomFloat( 0.05f, 0.25f ), "ZapThink" );
	}
	else
	{
		SetContextThink( NULL, gpGlobals->curtime, "ZapThink" );
...
void CNPC_AntlionWorker::Ignite ( float flFlameLifetime, bool bNPCOnly, float flSize, bool bCalledByLevelDesigner )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
int CNPC_AntlionWorker::OnTakeDamage_Alive( const CTakeDamageInfo &info )
{
	CTakeDamageInfo newInfo = info;

	if (antlion_worker_easycrush.GetBool())
	{
...
//-----------------------------------------------------------------------------
void CNPC_AntlionWorker::CascadePush( const Vector &vecForce )
{
	...
	for ( int i = 0; i < nNumEnemies; i++ )
	{
		...
		CNPC_AntlionWorker *pAntlion = dynamic_cast<CNPC_AntlionWorker *>(pEnemySearch[i]);
		if ( pAntlion != NULL )
		{
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
inline bool CNPC_AntlionWorker::IsFlipped( void ) 
{
	return ( GetActivity() == ACT_WNTLION_FLIP || GetActivity() == ACT_WNTLION_ZAP_FLIP );
}

//-----------------------------------------------------------------------------
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_AntlionWorker::TraceAttack( const CTakeDamageInfo &info, const Vector &vecDir, trace_t *ptr, CDmgAccumulator *pAccumulator )
{
	CTakeDamageInfo newInfo = info;
		...
		if ( !IsRunningDynamicInteraction() )
 		{
			...
			if( hl2_episodic.GetBool() )
			{
				...
				if( GetFlags() & FL_ONGROUND )
				{
					...
					SetCondition( COND_ANTLIONWORKER_FLIPPED );
				}
...
void CNPC_AntlionWorker::StopLoopingSounds( void )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_AntlionWorker::IdleSound( void )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_AntlionWorker::PainSound( const CTakeDamageInfo &info )
{
	if (IsOnFire())
		return;
...
// Output : 
//-----------------------------------------------------------------------------
float CNPC_AntlionWorker::GetIdealAccel( void ) const
{
...
// Output : float
//-----------------------------------------------------------------------------
float CNPC_AntlionWorker::MaxYawSpeed( void )
{
	switch ( GetActivity() )
	{
	case ACT_IDLE:		
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CNPC_AntlionWorker::ShouldPlayIdleSound( void )
{
...
// Input  : *pFriend - 
//-----------------------------------------------------------------------------
void CNPC_AntlionWorker::NotifyDeadFriend( CBaseEntity *pFriend )
{
	SetCondition( COND_ANTLIONWORKER_SQUADMATE_KILLED );
...
//-----------------------------------------------------------------------------
bool CNPC_AntlionWorker::FCanCheckAttacks( void )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
int CNPC_AntlionWorker::RangeAttack1Conditions( float flDot, float flDist )
{
	if ( GetNextAttack() > gpGlobals->curtime )
...
// Purpose: 
//-----------------------------------------------------------------------------
int CNPC_AntlionWorker::MeleeAttack1Conditions( float flDot, float flDist )
{
...
// Output : int
//-----------------------------------------------------------------------------
int CNPC_AntlionWorker::MeleeAttack2Conditions( float flDot, float flDist )
{
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CNPC_AntlionWorker::Alone( void )
{
	if ( m_pSquad == NULL )
		return true;
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_AntlionWorker::StartJump( void )
{
...
//-----------------------------------------------------------------------------
bool CNPC_AntlionWorker::FValidateHintType( CAI_Hint *pHint )
{
	switch ( m_iContext )
	{
...
// Input  : &origin - 
//-----------------------------------------------------------------------------
void CNPC_AntlionWorker::ClearBurrowPoint( const Vector &origin )
{
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CNPC_AntlionWorker::ValidBurrowPoint( const Vector &point )
{
	trace_t	tr;
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CNPC_AntlionWorker::FindBurrow( const Vector &origin, float distance, int type, bool excludeNear )
{
	//Burrowing in?
...
void CNPC_AntlionWorker::BurrowUse( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
	...
	SetSchedule( SCHED_ANTLIONWORKER_WAIT_UNBORROW );
}

//-----------------------------------------------------------------------------
// Purpose: Monitor the antlion's jump to play the proper landing sequence
//-----------------------------------------------------------------------------
bool CNPC_AntlionWorker::CheckLanding( void )
{
	trace_t	tr;
	Vector	testPos;
	...
	{
		int	sequence = SelectWeightedSequence( (Activity)ACT_WNTLION_LAND );

		if ( GetSequence() != sequence )
		{
			SetWings( false );
			VacateStrategySlot();
			SetIdealActivity( (Activity) ACT_WNTLION_LAND );
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CNPC_AntlionWorker::QuerySeeEntity( CBaseEntity *pEntity, bool bOnlyHateOrFearIfNPC )
{
...
//-----------------------------------------------------------------------------
void CNPC_AntlionWorker::SetWings( bool state )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_AntlionWorker::Burrow( void )
{
	SetWings( false );
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_AntlionWorker::Unburrow( void )
{
	m_bStartBurrowed = false;
	SetWings( false );
...
// Input  : &inputdata - 
//-----------------------------------------------------------------------------
void CNPC_AntlionWorker::InputUnburrow( inputdata_t &inputdata )
{
	if ( IsAlive() == false )
		return;
...
// Input  : &inputdata - 
//-----------------------------------------------------------------------------
void CNPC_AntlionWorker::InputBurrow( inputdata_t &inputdata )
{
	if ( IsAlive() == false )
		return;

	SetSchedule( SCHED_ANTLIONWORKER_BURROW_IN );
}
...
// Input  : &inputdata - 
//-----------------------------------------------------------------------------
void CNPC_AntlionWorker::InputBurrowAway( inputdata_t &inputdata )
{
	if ( IsAlive() == false )
		return;

	SetSchedule( SCHED_ANTLIONWORKER_BURROW_AWAY );
}

//-----------------------------------------------------------------------------
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_AntlionWorker::CreateDust( bool placeDecal )
{
	trace_t	tr;
...
// Input  : *pSound - 
//-----------------------------------------------------------------------------
bool CNPC_AntlionWorker::QueryHearSound( CSound *pSound )
{
...
//-----------------------------------------------------------------------------
void CNPC_AntlionWorker::BuildScheduleTestBits( void )
{
	...
	{
		SetCustomInterruptCondition( COND_ANTLIONWORKER_CAN_JUMP );
		SetCustomInterruptCondition( COND_ENEMY_UNREACHABLE );
	}

	if ( !IsCurSchedule( SCHED_ANTLIONWORKER_DROWN ) )
	{
		// Interrupt any schedule unless already drowning.
		SetCustomInterruptCondition( COND_ANTLIONWORKER_IN_WATER );
	}
	else
	{
		// Don't stop drowning just because you're in water!
		ClearCustomInterruptCondition( COND_ANTLIONWORKER_IN_WATER );
	}
	...
	*/
	...
	if( !IsCurSchedule(SCHED_ANTLIONWORKER_FLEE_THUMPER)			&& 		
		!IsCurSchedule(SCHED_ANTLIONWORKER_FLEE_PHYSICS_DANGER)	&& 		
		!IsCurSchedule(SCHED_ANTLIONWORKER_BURROW_IN)				&& 		
		!IsCurSchedule(SCHED_ANTLIONWORKER_WAIT_UNBORROW)			&& 		
		!IsCurSchedule(SCHED_ANTLIONWORKER_BURROW_OUT)			&&
		!IsCurSchedule(SCHED_ANTLIONWORKER_BURROW_WAIT)			&&
		!IsCurSchedule(SCHED_ANTLIONWORKER_WAIT_FOR_UNBORROW_TRIGGER)&&
		!IsCurSchedule(SCHED_ANTLIONWORKER_WAIT_FOR_CLEAR_UNBORROW)&&
		!IsCurSchedule(SCHED_ANTLIONWORKER_WAIT_UNBORROW)			&&
		!IsCurSchedule(SCHED_ANTLIONWORKER_JUMP)					&&
		!IsCurSchedule(SCHED_ANTLIONWORKER_FLIP)					&&
		!IsCurSchedule(SCHED_ANTLIONWORKER_DISMOUNT_NPC)			&& 
		( GetFlags() & FL_ONGROUND ) )
	{
		// Only do these if not jumping as well
		if (!IsCurSchedule(SCHED_ANTLIONWORKER_JUMP))
		{
			if ( GetEnemy() == NULL )
			{
			...
			SetCustomInterruptCondition( COND_ANTLIONWORKER_FLIPPED );
			SetCustomInterruptCondition( COND_ANTLIONWORKER_CAN_JUMP_AT_TARGET );

			if ( GetNavType() != NAV_JUMP )
				 SetCustomInterruptCondition( COND_ANTLIONWORKER_RECEIVED_ORDERS );
		}

		SetCustomInterruptCondition( COND_ANTLIONWORKER_ON_NPC );
	}
}
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CNPC_AntlionWorker::IsValidEnemy( CBaseEntity *pEnemy )
{
	...
	if ( IsCurSchedule( GetGlobalScheduleId( SCHED_ANTLIONWORKER_CHASE_BUGBAIT ) ) && ( GetNavigator() != NULL ) )
	{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_AntlionWorker::GatherConditions( void )
{
	...
	{
		SetCondition( COND_ANTLIONWORKER_ON_NPC );
	}
	else
	{
		ClearCondition( COND_ANTLIONWORKER_ON_NPC );
	}
...
/*	if ( m_hFollowTarget != NULL )
	{
		...
		if ( targetDist > 400 )
		{
			SetCondition( COND_ANTLIONWORKER_FOLLOW_TARGET_TOO_FAR );
		}
		else
		{
			ClearCondition( COND_ANTLIONWORKER_FOLLOW_TARGET_TOO_FAR );
		}
	}*/

	if ( IsCurSchedule( SCHED_ANTLIONWORKER_BURROW_WAIT ) == false && 
		 IsCurSchedule(SCHED_ANTLIONWORKER_BURROW_IN) == false && 
		 IsCurSchedule(SCHED_ANTLIONWORKER_BURROW_OUT) == false && 
		 ...
		 IsEffectActive( EF_NODRAW ) == false )
	{
		...
		{
			// Start Drowning!
			SetCondition( COND_ANTLIONWORKER_IN_WATER );
		}
	}

	//Ignore the player pushing me if I'm flipped over!
	if ( IsCurSchedule( SCHED_ANTLIONWORKER_FLIP ) )
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_AntlionWorker::PrescheduleThink( void )
{
	UpdateHead();
	...
	if ( ( eActivity == ACT_WNTLION_RUN_AGITATED ) && ( m_bAgitatedSound == false ) )
	{
		//Start sound
	...
	else if ( ( eActivity != ACT_WNTLION_RUN_AGITATED ) && ( m_bAgitatedSound == true ) )
	{
		//Stop sound
		StopSound( "NPC_Antlion.LoopingAgitated" );
		m_bAgitatedSound = false;
	...
	if (    ( m_bWingsOpen ) &&
			( eActivity != ACT_WNTLION_JUMP_START ) && 
			( eActivity != ACT_JUMP ) && 
			( eActivity != ACT_GLIDE ) && 
			( eActivity != ACT_WNTLION_LAND ) && 
			( eActivity != ACT_WNTLION_DISTRACT ))
	{
		SetWings( false );
	}
	...
	if ( IsEffectActive( EF_NODRAW ) &&
		 ( eActivity != ACT_WNTLION_BURROW_IDLE ) &&
		 ( eActivity != ACT_WNTLION_BURROW_OUT ) &&
		 ( eActivity != ACT_WNTLION_BURROW_IN) )
	{
	...
	if ( ShouldJump() )
	{
		SetCondition( COND_ANTLIONWORKER_CAN_JUMP );
	}
	else
	{
		ClearCondition( COND_ANTLIONWORKER_CAN_JUMP );
	}
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CNPC_AntlionWorker::IsLightDamage( const CTakeDamageInfo &info )
{
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CNPC_AntlionWorker::IsAllied( void )
{
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CNPC_AntlionWorker::ShouldResumeFollow( void )
{
	if ( IsAllied() == false )
		return false;

	if ( m_MoveState == ANTLIONWORKER_MOVE_FOLLOW || m_hFollowTarget == NULL )
		return false;
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CNPC_AntlionWorker::ShouldAbandonFollow( void )
{
...
// Input  : *pTarget - 
//-----------------------------------------------------------------------------
void CNPC_AntlionWorker::SetFightTarget( CBaseEntity *pTarget )
{
	m_hFightGoalTarget = pTarget;

	SetCondition( COND_ANTLIONWORKER_RECEIVED_ORDERS );
}
...
// Input  : &inputdata - 
//-----------------------------------------------------------------------------
void CNPC_AntlionWorker::InputFightToPosition( inputdata_t &inputdata )
{
	if ( IsAlive() == false )
		return;
...
// Input  : &inputdata - 
//-----------------------------------------------------------------------------
void CNPC_AntlionWorker::InputStopFightToPosition( inputdata_t &inputdata )
{
	SetFightTarget( NULL );
}
...
// Input  : *pEnemy - 
//-----------------------------------------------------------------------------
void CNPC_AntlionWorker::GatherEnemyConditions( CBaseEntity *pEnemy )
{
	// Do the base class
	BaseClass::GatherEnemyConditions( pEnemy );

	// Only continue if we burrow when eluded
	if ((m_spawnflags & SF_ANTLIONWORKER_BURROW_ON_ELUDED) == false)
		return;
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CNPC_AntlionWorker::ShouldGib( const CTakeDamageInfo &info )
{
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CNPC_AntlionWorker::CorpseGib( const CTakeDamageInfo &info )
{
	DoPoisonBurst();
...
// Input  : *pOther - 
//-----------------------------------------------------------------------------
void CNPC_AntlionWorker::Touch( CBaseEntity *pOther )
{
	...
	for (int i = 1; i <= gpGlobals->maxClients; i++)
	{
		...
		if (pVehicleEnt == pOther)
		{
			...
			if (pDrivableVehicle != NULL)
			{
				...
				bool bBurrowingOut = IsCurSchedule(SCHED_ANTLIONWORKER_BURROW_OUT);
	...
	{
		CTakeDamageInfo	dmgInfo(this, this, sk_antlion_worker_air_attack_dmg.GetInt(), DMG_SLASH);
	...
	if (pOther->IsPlayer())
	{
		...
		if (!IsCurSchedule(SCHED_MOVE_AWAY) && !IsCurSchedule(SCHED_ANTLIONWORKER_BURROW_OUT))
...
// Output :
//-----------------------------------------------------------------------------
bool CNPC_AntlionWorker::OverrideMoveFacing( const AILocalMoveGoal_t &move, float flInterval )
{
	if ( GetEnemy() )
	{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_AntlionWorker::InputDisableJump( inputdata_t &inputdata )
{
	m_bDisableJump = true;
	CapabilitiesRemove( bits_CAP_MOVE_JUMP );
}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_AntlionWorker::InputEnableJump( inputdata_t &inputdata )
{
	m_bDisableJump = false;
	CapabilitiesAdd( bits_CAP_MOVE_JUMP );
}
...
// Input  : *pTarget - 
//-----------------------------------------------------------------------------
void CNPC_AntlionWorker::SetFollowTarget( CBaseEntity *pTarget )
{
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CNPC_AntlionWorker::CreateBehaviors( void )
{
...
// Input  : &inputdata - 
//-----------------------------------------------------------------------------
void CNPC_AntlionWorker::InputIgnoreBugbait( inputdata_t &inputdata )
{
...
// Input  : &inputdata - 
//-----------------------------------------------------------------------------
void CNPC_AntlionWorker::InputHearBugbait( inputdata_t &inputdata )
{
...
// Input  : state - 
//-----------------------------------------------------------------------------
void CNPC_AntlionWorker::SetMoveState( AntlionWorkerMoveState_e state )
{
	m_MoveState = state;
...
//-----------------------------------------------------------------------------
Vector CNPC_AntlionWorker::BodyTarget( const Vector &posSrc, bool bNoisy /*= true*/ )
{ 
...
//-----------------------------------------------------------------------------
void CNPC_AntlionWorker::Flip( bool bZapped /*= false*/ )
{
	...
	if ( bZapped )
	{
		m_flZapDuration = gpGlobals->curtime + SequenceDuration( SelectWeightedSequence( (Activity) ACT_WNTLION_ZAP_FLIP) ) + 0.1f;
...
// Input  : &inputdata - 
//-----------------------------------------------------------------------------
void CNPC_AntlionWorker::InputJumpAtTarget( inputdata_t &inputdata )
{
	...
	SetCondition( COND_ANTLIONWORKER_CAN_JUMP_AT_TARGET );
}

//-----------------------------------------------------------------------------
// workers can explode.
//-----------------------------------------------------------------------------
void CNPC_AntlionWorker::DoPoisonBurst()
{
	ConVar *sk_antlion_worker_burst_radius = cvar->FindVar("sk_antlion_worker_burst_radius");
	ConVar *sk_antlion_worker_burst_damage = cvar->FindVar("sk_antlion_worker_burst_damage");
...
// Purpose: 
//-----------------------------------------------------------------------------
bool CNPC_AntlionWorker::IsHeavyDamage( const CTakeDamageInfo &info )
{
...
AI_BEGIN_CUSTOM_NPC( npc_antlionworker, CNPC_AntlionWorker )
	...
	DECLARE_CONDITION( COND_ANTLIONWORKER_ON_NPC )
	DECLARE_CONDITION( COND_ANTLIONWORKER_CAN_JUMP )
	DECLARE_CONDITION( COND_ANTLIONWORKER_FOLLOW_TARGET_TOO_FAR )
	DECLARE_CONDITION( COND_ANTLIONWORKER_RECEIVED_ORDERS )
	DECLARE_CONDITION( COND_ANTLIONWORKER_IN_WATER )
	DECLARE_CONDITION( COND_ANTLIONWORKER_CAN_JUMP_AT_TARGET )
	DECLARE_CONDITION( COND_ANTLIONWORKER_SQUADMATE_KILLED )
		
	//Squad slots
	DECLARE_SQUADSLOT( SQUAD_SLOT_ANTLIONWORKER_JUMP )
	DECLARE_SQUADSLOT( SQUAD_SLOT_ANTLIONWORKER_FIRE )

	//Tasks
	DECLARE_TASK( TASK_ANTLIONWORKER_SET_CHARGE_GOAL )
	DECLARE_TASK( TASK_ANTLIONWORKER_BURROW )
	DECLARE_TASK( TASK_ANTLIONWORKER_UNBURROW )
	DECLARE_TASK( TASK_ANTLIONWORKER_VANISH )
	DECLARE_TASK( TASK_ANTLIONWORKER_FIND_BURROW_IN_POINT )
	DECLARE_TASK( TASK_ANTLIONWORKER_FIND_BURROW_OUT_POINT )
	DECLARE_TASK( TASK_ANTLIONWORKER_BURROW_WAIT )
	DECLARE_TASK( TASK_ANTLIONWORKER_CHECK_FOR_UNBORROW )
	DECLARE_TASK( TASK_ANTLIONWORKER_JUMP )
	DECLARE_TASK( TASK_ANTLIONWORKER_WAIT_FOR_TRIGGER )
	DECLARE_TASK( TASK_ANTLIONWORKER_GET_THUMPER_ESCAPE_PATH )
	DECLARE_TASK( TASK_ANTLIONWORKER_GET_PATH_TO_BUGBAIT )
	DECLARE_TASK( TASK_ANTLIONWORKER_FACE_BUGBAIT )
	DECLARE_TASK( TASK_ANTLIONWORKER_DISMOUNT_NPC )
	DECLARE_TASK( TASK_ANTLIONWORKER_REACH_FIGHT_GOAL )
	DECLARE_TASK( TASK_ANTLIONWORKER_GET_PHYSICS_DANGER_ESCAPE_PATH )
	DECLARE_TASK( TASK_ANTLIONWORKER_FACE_JUMP )
	DECLARE_TASK( TASK_ANTLIONWORKER_DROWN )
	DECLARE_TASK( TASK_ANTLIONWORKER_GET_PATH_TO_RANDOM_NODE )
	DECLARE_TASK( TASK_ANTLIONWORKER_FIND_COVER_FROM_SAVEPOSITION )

	//Activities
	DECLARE_ACTIVITY( ACT_WNTLION_DISTRACT )
	DECLARE_ACTIVITY( ACT_WNTLION_DISTRACT_ARRIVED )
	DECLARE_ACTIVITY( ACT_WNTLION_JUMP_START )
	DECLARE_ACTIVITY( ACT_WNTLION_BURROW_IN )
	DECLARE_ACTIVITY( ACT_WNTLION_BURROW_OUT )
	DECLARE_ACTIVITY( ACT_WNTLION_BURROW_IDLE )
	DECLARE_ACTIVITY( ACT_WNTLION_RUN_AGITATED )
	DECLARE_ACTIVITY( ACT_WNTLION_FLIP )
	DECLARE_ACTIVITY( ACT_WNTLION_POUNCE )
	DECLARE_ACTIVITY( ACT_WNTLION_POUNCE_MOVING )
	DECLARE_ACTIVITY( ACT_WNTLION_DROWN )
	DECLARE_ACTIVITY( ACT_WNTLION_LAND )
	DECLARE_ACTIVITY( ACT_WNTLION_WORKER_EXPLODE )
	DECLARE_ACTIVITY( ACT_WNTLION_ZAP_FLIP )

	//Events
	DECLARE_ANIMEVENT( AE_WNTLION_WALK_FOOTSTEP )
	DECLARE_ANIMEVENT( AE_WNTLION_MELEE_HIT1 )
	DECLARE_ANIMEVENT( AE_WNTLION_MELEE_HIT2 )
	DECLARE_ANIMEVENT( AE_WNTLION_MELEE_POUNCE )
	DECLARE_ANIMEVENT( AE_WNTLION_FOOTSTEP_SOFT )
	DECLARE_ANIMEVENT( AE_WNTLION_FOOTSTEP_HEAVY )
	DECLARE_ANIMEVENT( AE_WNTLION_START_JUMP )
	DECLARE_ANIMEVENT( AE_WNTLION_BURROW_IN )
	DECLARE_ANIMEVENT( AE_WNTLION_BURROW_OUT )
	DECLARE_ANIMEVENT( AE_WNTLION_VANISH )
	DECLARE_ANIMEVENT( AE_WNTLION_OPEN_WINGS )
	DECLARE_ANIMEVENT( AE_WNTLION_CLOSE_WINGS )
	DECLARE_ANIMEVENT( AE_WNTLION_MELEE1_SOUND )
	DECLARE_ANIMEVENT( AE_WNTLION_MELEE2_SOUND )
	DECLARE_ANIMEVENT( AE_WNTLION_WORKER_EXPLODE_SCREAM )
	DECLARE_ANIMEVENT( AE_WNTLION_WORKER_EXPLODE_WARN )
	DECLARE_ANIMEVENT( AE_WNTLION_WORKER_EXPLODE )
	DECLARE_ANIMEVENT( AE_WNTLION_WORKER_SPIT )
	DECLARE_ANIMEVENT( AE_WNTLION_WORKER_DONT_EXPLODE )
	...
	(
		...
		"	Tasks"
		"		TASK_STOP_MOVING				0"
		"		TASK_ANTLIONWORKER_FACE_JUMP			0"
		"		TASK_PLAY_SEQUENCE				ACTIVITY:ACT_WNTLION_JUMP_START"
		"		TASK_ANTLIONWORKER_JUMP				0"
		""
		"	Interrupts"
		"		COND_TASK_FAILED"
	)
	...
	(
		...
		"	Tasks"
		"		TASK_ANTLIONWORKER_BURROW_WAIT		0"
		"		TASK_SET_SCHEDULE				SCHEDULE:SCHED_ANTLIONWORKER_WAIT_FOR_CLEAR_UNBORROW"
		""
		"	Interrupts"
		"		COND_TASK_FAILED"
	)
	...
	(
		...
		"	Tasks"
		"		TASK_SET_FAIL_SCHEDULE				SCHEDULE:SCHED_ANTLIONWORKER_BURROW_WAIT"
		"		TASK_ANTLIONWORKER_BURROW_WAIT			1"
		"		TASK_ANTLIONWORKER_FIND_BURROW_OUT_POINT	1024"
		"		TASK_SET_SCHEDULE					SCHEDULE:SCHED_ANTLIONWORKER_WAIT_FOR_CLEAR_UNBORROW"
		""
		"	Interrupts"
		"		COND_TASK_FAILED"
	...
	(
		...
		"		TASK_ANTLIONWORKER_BURROW					0"
		"		TASK_PLAY_SEQUENCE					ACTIVITY:ACT_WNTLION_BURROW_IN"
		"		TASK_ANTLIONWORKER_VANISH					0"
		"		TASK_SET_SCHEDULE					SCHEDULE:SCHED_ANTLIONWORKER_BURROW_WAIT"
		""
		"	Interrupts"
		"		COND_TASK_FAILED"
	)
	...
	(
		...
		"		TASK_ANTLIONWORKER_FIND_BURROW_IN_POINT	512"
		"		TASK_RUN_PATH						0"
		"		TASK_WAIT_FOR_MOVEMENT				0"
		"		TASK_SET_SCHEDULE					SCHEDULE:SCHED_ANTLIONWORKER_BURROW_IN"
		""
		"	Interrupts"
		"		COND_TASK_FAILED"
		"		COND_GIVE_WAY"
	...
	(
		...
		"	Tasks"
		"		TASK_SET_FAIL_SCHEDULE			SCHEDULE:SCHED_ANTLIONWORKER_BURROW_WAIT"
		"		TASK_ANTLIONWORKER_UNBURROW			0"
		"		TASK_PLAY_SEQUENCE				ACTIVITY:ACT_WNTLION_BURROW_OUT"
		""
		"	Interrupts"
		"		COND_TASK_FAILED"
	)
	...
	(
		...
		"	Tasks"
		"		TASK_ANTLIONWORKER_WAIT_FOR_TRIGGER	0"
		""
		"	Interrupts"
		"		COND_TASK_FAILED"
	)
	...
	(
		...
		"	Tasks"
		"		TASK_SET_FAIL_SCHEDULE				SCHEDULE:SCHED_ANTLIONWORKER_BURROW_WAIT"
		"		TASK_ANTLIONWORKER_CHECK_FOR_UNBORROW		1"
		"		TASK_SET_SCHEDULE					SCHEDULE:SCHED_ANTLIONWORKER_BURROW_OUT"
		""
		"	Interrupts"
		"		COND_TASK_FAILED"
	)
	...
	(
		...
		"		TASK_ANTLIONWORKER_GET_THUMPER_ESCAPE_PATH	0"
		"		TASK_RUN_PATH							0"
		"		TASK_WAIT_FOR_MOVEMENT					0"
		"		TASK_STOP_MOVING						0"
		"		TASK_PLAY_SEQUENCE						ACTIVITY:ACT_WNTLION_DISTRACT_ARRIVED"
		""
		"	Interrupts"
		"		COND_TASK_FAILED"
		"		COND_ANTLIONWORKER_FLIPPED"
	)
	...
	(
		...
		"	Tasks"
		"		TASK_STOP_MOVING					0"
		"		TASK_ANTLIONWORKER_GET_PATH_TO_BUGBAIT	0"
		"		TASK_RUN_PATH						0"
		"		TASK_WAIT_FOR_MOVEMENT				0"
		"		TASK_STOP_MOVING					0"
		"		TASK_ANTLIONWORKER_FACE_BUGBAIT			0"
		""
		"	Interrupts"
		"		COND_CAN_MELEE_ATTACK1"
		"		COND_SEE_ENEMY"
	...
	(
		...
		"		TASK_PLAY_SEQUENCE		ACTIVITY:ACT_WNTLION_ZAP_FLIP"
	...
	"		TASK_STOP_MOVING	0"
	"		TASK_RESET_ACTIVITY		0"
	"		TASK_PLAY_SEQUENCE		ACTIVITY:ACT_WNTLION_FLIP"
	...
	(
		...
		"	Tasks"
		"		TASK_STOP_MOVING			0"
		"		TASK_ANTLIONWORKER_DISMOUNT_NPC	0"
	...
	(
		...
		"		TASK_RUN_PATH					0"
		"		TASK_WAIT_FOR_MOVEMENT			0"
		"		TASK_ANTLIONWORKER_REACH_FIGHT_GOAL	0"
	...
	(
		...
		"		TASK_ANTLIONWORKER_GET_PHYSICS_DANGER_ESCAPE_PATH	1024"
		"		TASK_RUN_PATH								0"
		"		TASK_WAIT_FOR_MOVEMENT						0"
		"		TASK_STOP_MOVING							0"
		""
	...
	(
		...
		"		TASK_RESET_ACTIVITY		0"
		"		TASK_PLAY_SEQUENCE		ACTIVITY:ACT_WNTLION_POUNCE"
	...
	(
		...
		"		TASK_ANNOUNCE_ATTACK	1"
		"		TASK_RESET_ACTIVITY		0"
		"		TASK_PLAY_SEQUENCE		ACTIVITY:ACT_WNTLION_POUNCE_MOVING"
	...
	(
		...
		"	Tasks"
		"		TASK_SET_ACTIVITY			ACTIVITY:ACT_WNTLION_DROWN"
		"		TASK_ANTLIONWORKER_DROWN			0"
		""
		"	Interrupts"
	)
	...
	(
		...
		"	Tasks"
		"		TASK_SET_FAIL_SCHEDULE					SCHEDULE:SCHED_ANTLIONWORKER_WORKER_RUN_RANDOM"
	...
	(
		...
		"	Tasks"
		"		TASK_SET_FAIL_SCHEDULE			SCHEDULE:SCHED_ANTLIONWORKER_TAKE_COVER_FROM_ENEMY"
		"		TASK_SET_TOLERANCE_DISTANCE		48"
		"		TASK_SET_ROUTE_SEARCH_TIME		1"	// Spend 1 second trying to build a path if stuck
	...
	(
		...
		"	Tasks"
		"		TASK_SET_FAIL_SCHEDULE						SCHEDULE:SCHED_FAIL_TAKE_COVER"
		"		TASK_ANTLIONWORKER_FIND_COVER_FROM_SAVEPOSITION	0"
		"		TASK_RUN_PATH								0"
		"		TASK_WAIT_FOR_MOVEMENT						0"
		"		TASK_STOP_MOVING							0"
		""
