...
ConVar	sk_elitepolice_health("sk_elitepolice_health", "0");
...
void CNPC_ElitePolice::Event_Killed(const CTakeDamageInfo &info)
{
	if (!(g_Language.GetInt() == LANGUAGE_GERMAN || UTIL_IsLowViolence()) && info.GetDamageType() & (DMG_BLAST | DMG_CRUSH) && !(info.GetDamageType() & (DMG_DISSOLVE)) && !PlayerHasMegaPhysCannon())
	{
		...
		CBaseEntity *pLeftArmGib = CreateRagGib(this, "models/gibs/elitepolice_left_arm.mdl", GetAbsOrigin(), GetAbsAngles(), vecRagDmgForce, flFadeTime, IsOnFire());
		if (pLeftArmGib)
		{
		...
		CBaseEntity *pRightArmGib = CreateRagGib(this, "models/gibs/elitepolice_right_arm.mdl", GetAbsOrigin(), GetAbsAngles(), vecRagDmgForce, flFadeTime, IsOnFire());
		if (pRightArmGib)
		{
		...
		CBaseEntity *pTorsoGib = CreateRagGib(this, "models/gibs/elitepolice_torso.mdl", GetAbsOrigin(), GetAbsAngles(), vecRagDmgForce, flFadeTime, IsOnFire());
		if (pTorsoGib)
		{
		...
		CBaseEntity *pPelvisGib = CreateRagGib(this, "models/gibs/elitepolice_pelvis.mdl", GetAbsOrigin(), GetAbsAngles(), vecRagDmgForce, flFadeTime, IsOnFire());
		if (pPelvisGib)
		{
		...
		CBaseEntity *pLeftLegGib = CreateRagGib(this, "models/gibs/elitepolice_left_leg.mdl", GetAbsOrigin(), GetAbsAngles(), vecRagDmgForce, flFadeTime, IsOnFire());
		if (pLeftLegGib)
		{
		...
		CBaseEntity *pRightLegGib = CreateRagGib(this, "models/gibs/elitepolice_right_leg.mdl", GetAbsOrigin(), GetAbsAngles(), vecRagDmgForce, flFadeTime, IsOnFire());
		if (pRightLegGib)
		{
